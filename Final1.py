...
numeric_feature_names = pipeline.named_steps['preprocessor'].transformers_[0][2].tolist()
categorical_feature_names = pipeline.named_steps['preprocessor'].named_transformers_['cat'] \
...
X_test_processed_df = pd.DataFrame(X_test_processed, columns=numeric_feature_names + categorical_feature_names)
...
interesting_features = ['Age', 'Flight Distance', 'Inflight wifi service', 'Seat comfort']
...
outliers = df[interesting_features][df[interesting_features].apply(lambda x: (x - x.mean()).abs() > 3 * x.std())]
print("Skewness:")
print(skewness)
print("\nOutliers:")
print(outliers)
...
print(f"Correlation between Inflight wifi service and Online boarding: {correlation_wifi_boarding}")

# Assumption 2: Higher 'Inflight entertainment' ratings for younger passengers
age_groups = pd.cut(df['Age'], bins=[0, 18, 30, 50, np.inf], labels=['<18', '18-30', '30-50', '50+'])
...
mean_wifi_business = df[df['Customer Type'] == 'Business travel']['Inflight wifi service'].mean()
mean_wifi_personal = df[df['Customer Type'] == 'Personal Travel']['Inflight wifi service'].mean()
...
correlation_distance_entertainment = df['Flight Distance'].corr(df['Inflight entertainment'])
print(f"Correlation between Flight Distance and Inflight entertainment: {correlation_distance_entertainment}")
